...
use super::{NixConfigType, NixOSConfig};
...
impl ConfigConverter {
    ...
    pub fn traditional_to_nixos(
        ...
        match target_type {
            NixConfigType::HomeManager => self.generate_home_manager_config(
                traditional_config,
                keybinds,
                window_rules,
                layer_rules,
            ...
            NixConfigType::FlakeHomeManager => self.generate_flake_home_manager_config(
                traditional_config,
                keybinds,
                window_rules,
                layer_rules,
            ),
            NixConfigType::FlakeSystem => self.generate_flake_system_config(
                traditional_config,
                keybinds,
                window_rules,
                layer_rules,
  ...
  outputs = { nixpkgs, hyprland, ... }: {
    ...
    fn convert_rule_to_nix(&self, rule: &str) -> String {
        ...
        if rule.starts_with("windowrule = ") {
            rule.strip_prefix("windowrule = ")
                .unwrap_or(rule)
                .to_string()
        } else if rule.starts_with("layerrule = ") {
            rule.strip_prefix("layerrule = ")
                .unwrap_or(rule)
                .to_string()
        } else {
            rule.to_string()
...
mod tests {
    ...
    fn test_traditional_to_nixos_conversion() {
        ...
        let window_rules = vec!["float, ^(kitty)$".to_string()];

        let layer_rules = vec!["blur, waybar".to_string()];
    ...
    fn test_value_conversion() {
        ...
        assert_eq!(
            converter.convert_value_to_nix("rgb(255, 0, 0)"),
            "\"rgb(255, 0, 0)\""
        );
        assert_eq!(
            converter.convert_value_to_nix("hello world"),
            "\"hello world\""
        );
    }
