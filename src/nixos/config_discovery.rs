...
use super::{NixOSConfig, NixConfigType, NixConfigLocation};
...
fn parse_traditional_nix_config(content: &str, location: &NixConfigLocation) -> Result<NixOSConfig, anyhow::Error> {
...
fn parse_flake_config(content: &str, location: &NixConfigLocation) -> Result<NixOSConfig, anyhow::Error> {
    let mut config = NixOSConfig {
        config_type: location.config_type.clone(),
        file_path: location.path.clone(),
        hyprland_settings: HashMap::new(),
    ...
    if let Some(start_pos) = content.find(&start_pattern) {
        ...
        if let Some(brace_pos) = content[start_pos..].find('{') {
            ...
            if let Some(end_pos) = find_matching_brace(&content[absolute_brace_pos..]) {
                let section_content = &content[absolute_brace_pos..absolute_brace_pos + end_pos + 1];
...
fn parse_settings_attribute_set(content: &str, config: &mut NixOSConfig) -> Result<(), anyhow::Error> {
...
fn parse_traditional_hyprland_config(content: &str, config: &mut NixOSConfig) -> Result<(), anyhow::Error> {
...
fn parse_array_value(value: &str, key: &str, config: &mut NixOSConfig) -> Result<(), anyhow::Error> {
