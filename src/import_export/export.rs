...
use super::formats::{
    AnimationSettings, ConfigFormat, DecorationSettings, GeneralSettings, GestureSettings,
    InputSettings, KeybindEntry, LayerRuleEntry, MiscSettings, StructuredConfig, WindowRuleEntry,
};
...
impl ConfigExporter {
    ...
    fn convert_to_structured(
        ...
        structured.gestures = GestureSettings {
            ...
            workspace_swipe_min_speed_to_force: Some(
                config.gestures.workspace_swipe_min_speed_to_force,
            ),
    ...
    async fn extract_current_config(
        ...
        config.general.border_size = options
    ...
    fn to_nix_home_manager(&self, config: &StructuredConfig) -> Result<String> {
        use crate::nixos::{NixConfigGenerator, NixConfigType, NixGenerationOptions};
    ...
    fn structured_to_hyprland_config(&self, config: &StructuredConfig) -> Result<HyprlandConfig> {
        ...
        let general = crate::hyprctl::GeneralConfig {
            ...
            col_active_border: config
                .general
                .col_active_border
                .clone()
                .unwrap_or_else(|| "rgba(33ccffee)".to_string()),
            col_inactive_border: config
                .general
                .col_inactive_border
                .clone()
        ...
        let window_rules = config
            .window_rules
            .iter()
            .map(|wr| format!("{}, {}", wr.rule, wr.window_identifier))
            .collect();

        let layer_rules = config
            .layer_rules
            .iter()
            .map(|lr| format!("{}, {}", lr.rule, lr.layer))
            .collect();
