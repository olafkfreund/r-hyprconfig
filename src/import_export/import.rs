...
impl ConfigImporter {
    ...
    pub async fn execute_import(
        ...
        let mut structured_config = self
            .parse_config_file(&main_config.path, main_config.format.clone())
            .await?;
        ...
        if options.merge_configs {
            for config in configs.iter().filter(|c| !c.is_main_config) {
                let additional = self
                    .parse_config_file(&config.path, config.format.clone())
                    .await?;
    ...
    async fn discover_configs(&self, source: &ImportSource) -> Result<Vec<DiscoveredConfig>> {
        match source {
            ImportSource::GitHub {
                ...
                let repo_path = self
                    .clone_repository(url, branch.as_deref(), tag.as_deref(), commit.as_deref())
                    .await?;
                self.scan_directory_for_configs(&repo_path, &repo_path)
            }
            ImportSource::LocalFolder { path } => self.scan_directory_for_configs(path, path),
            ImportSource::LocalFile { path } => self.scan_single_file(path),
    ...
    async fn clone_repository(
        ...
        let target_path = self
            .temp_dir
            .join(format!("{}_{}", repo_name, uuid::Uuid::new_v4()));
    ...
    fn scan_directory_for_configs(
        &self,
        dir_path: &Path,
        base_path: &Path,
    ) -> Result<Vec<DiscoveredConfig>> {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if is_config_file {
                ...
                let is_main_config = config_patterns
                    .iter()
                    .any(|pattern| filename.eq_ignore_ascii_case(pattern))
                    || common_paths
                        .iter()
                        .any(|common_path| relative_path.starts_with(common_path));
    ...
    fn scan_single_file(&self, file_path: &Path) -> Result<Vec<DiscoveredConfig>> {
        ...
        let format =
            ConfigFormat::from_path(&file_path.to_path_buf()).unwrap_or(ConfigFormat::HyprlandConf);
    ...
    fn find_associated_files(&self, dir: &Path) -> Result<Vec<PathBuf>> {
        ...
        let asset_extensions = [
            // Images
            "jpg", "jpeg", "png", "webp", "bmp", "gif", "svg", // Scripts
            "sh", "bash", "zsh", "fish", "py", "lua", // Themes
            "css", "scss", "theme", // Other config files
            "json", "toml", "yaml", "yml",
        ];
    ...
    fn parse_rhypr_config(&self, content: &str) -> Result<StructuredConfig> {
        ...
        serde_json::from_str(json_content).context("Failed to parse r-hyprconfig JSON content")
    }

    // Helper parsing methods would go here...
    fn parse_general_setting(
        &self,
        config: &mut StructuredConfig,
        key: &str,
        value: &str,
    ) -> Result<()> {
        match key {
            ...
            "general:col.active_border" => {
                config.general.col_active_border = Some(value.to_string())
            }
            "general:col.inactive_border" => {
                config.general.col_inactive_border = Some(value.to_string())
            }
            _ => {
                config
                    .custom_settings
    ...
    fn determine_asset_target_path(
        &self,
        _source_path: &Path,
    ...
    fn merge_configs(
        &self,
        base: &mut StructuredConfig,
        additional: StructuredConfig,
    ) -> Result<()> {
        // Merge keybinds
    ...
    async fn import_assets(&self, assets: &[AssetInfo]) -> Result<()> {
        for asset in assets {
            ...
            std::fs::copy(&asset.path, &target_file).with_context(|| {
                format!(
                    "Failed to copy asset from {:?} to {:?}",
                    asset.path, target_file
                )
            })?;
        }
        Ok(())
...
"#;
    ...
    fn test_classify_asset() {
        ...
        assert_eq!(
            importer.classify_asset(Path::new("wallpaper.jpg")),
            Some(AssetType::Wallpaper)
        );
        assert_eq!(
            importer.classify_asset(Path::new("script.sh")),
            Some(AssetType::Script)
        );
        assert_eq!(
            importer.classify_asset(Path::new("font.ttf")),
            Some(AssetType::Font)
        );
        assert_eq!(
            importer.classify_asset(Path::new("theme.css")),
            Some(AssetType::Theme)
        );
        assert_eq!(
            importer.classify_asset(Path::new("readme.txt")),
