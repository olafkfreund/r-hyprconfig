...
impl Theme {
    ...
    pub fn header_style(&self) -> Style {
        Style::default().fg(self.accent_primary).bg(self.bg_primary)
    }

    pub fn tab_style(&self, focused: bool) -> Style {
        if focused {
            Style::default().fg(self.bg_primary).bg(self.accent_primary)
        } else {
    ...
    pub fn search_style(&self, active: bool) -> Style {
        if active {
            Style::default().fg(self.accent_warning).bg(self.bg_search)
        } else {
            Style::default().fg(self.fg_muted).bg(self.bg_secondary)
        }
    }
